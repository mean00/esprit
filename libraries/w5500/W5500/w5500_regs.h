/**
 * @file    w5500_regs.h
 * @brief   W5500 register address definitions.
 *
 * Defines the 24-bit register addresses for all W5500 common registers
 * (MR, GAR, SUBR, SHAR, SIPR, etc.) and socket registers (Sn_MR, Sn_CR,
 * Sn_SR, Sn_IR, Sn_PORT, Sn_DIPR, Sn_DPORT, etc.), plus the interrupt
 * register bit definitions and buffer block macros.
 *
 * @copyright (c) 2013, WIZnet Co., LTD.
 * @license  BSD-3-Clause (see full license in source)
 */

// MEANX : Cleanup / constify
#pragma once
...
/**
    ...
    - @ref WIZCHIP_IO_Functions : @ref Basic_IO_function, @ref Common_register_access_function and @ref
...
/**
    ...
    -# @b PPPoE \n
      getPTIMER(), setPTIMER(), getPMAGIC(), getPMAGIC(), getPSID(), setPSID(), getPHAR(), setPHAR(), getPMRU(),
   setPMRU()
    -# <b> ICMP packet </b>\n
      getUIPR(), getUPORTR()
    -# @b etc. \n
    ...
    - \ref Socket_register_group <b>access functions</b> \n
     ...
     -# <b> SOCKET information</b> \n
        getSn_SR(), getSn_DHAR(), setSn_DHAR(), getSn_PORT(), setSn_PORT(), getSn_DIPR(), setSn_DIPR(), getSn_DPORT(),
...
/**
    ...
    <table>
        ...
        <tr>  <td>RST</td> <td>Reserved</td> <td>WOL</td> <td>PB</td> <td>PPPoE</td> <td>Reserved</td> <td>FARP</td>
   <td>Reserved</td> </tr>
    </table>
    - \ref MR_RST		 	: Reset
...
/**
    ...
    @details @ref IR indicates the interrupt status. Each bit of @ref IR will be still until the bit will be written to
   by the host. If @ref IR is not equal to x00 INTn PIN is asserted to low until it is x00\n\n Each bit of @ref IR
   defined as follows. <table> <tr>  <td>7</td> <td>6</td> <td>5</td> <td>4</td> <td>3</td> <td>2</td> <td>1</td>
   <td>0</td>   </tr> <tr>  <td>CONFLICT</td> <td>UNREACH</td> <td>PPPoE</td> <td>MP</td> <td>Reserved</td>
...
/**
    ...
    <table>
        <tr>  <td>7</td> <td>6</td> <td>5</td> <td>4</td> <td>3</td> <td>2</td> <td>1</td> <td>0</td>   </tr>
        <tr>  <td>IM_IR7</td> <td>IM_IR6</td> <td>IM_IR5</td> <td>IM_IR4</td> <td>Reserved</td> <td>Reserved</td>
   <td>Reserved</td> <td>Reserved</td> </tr>
    </table>
...
/**
    ...
    In other words, if a bit of @ref SIMR is  an interrupt will be not issued even if the corresponding bit of @ref SIR
   is
*/
...
/**
    ...
    @details @ref _RTR_ configures the retransmission timeout period. The unit of timeout period is 100us and the
   default of @ref _RTR_ is x07D0. And so the default timeout period is 200ms(100us X 2000). During the time configured
   by @ref _RTR_, W5500 waits for the peer response to the packet that is transmitted by \ref Sn_CR (CONNECT, DISCON,
   CLOSE, SEND, SEND_MAC, SEND_KEEP command). If the peer does not respond within the @ref _RTR_ time, W5500 retransmits
   the packet or issues timeout.
*/
// M20150401 : Rename SYMBOE ( Re-define error in a compile)
...
/**
    ...
    @details @ref PHYCFGR configures PHY operation mode and resets PHY. In addition, @ref PHYCFGR indicates the status
   of PHY such as duplex, Speed, Link.
*/
...
/**
    ...
    <table>
        <tr>  <td>7</td> <td>6</td> <td>5</td> <td>4</td> <td>3</td> <td>2</td> <td>1</td> <td>0</td>   </tr>
        <tr>  <td>MULTI/MFEN</td> <td>BCASTB</td> <td>ND/MC/MMB</td> <td>UCASTB/MIP6B</td> <td>Protocol[3]</td>
   <td>Protocol[2]</td> <td>Protocol[1]</td> <td>Protocol[0]</td> </tr>
    ...
    - <b>Protocol</b>
    <table>
        <tr>   <td><b>Protocol[3]</b></td> <td><b>Protocol[2]</b></td> <td><b>Protocol[1]</b></td>
   <td><b>Protocol[0]</b></td> <td>@b Meaning</td>   </tr> <tr>   <td>0</td> <td>0</td> <td>0</td> <td>0</td>
   <td>Closed</td>   </tr> <tr>   <td>0</td> <td>0</td> <td>0</td> <td>1</td> <td>TCP</td>   </tr> <tr>   <td>0</td>
   <td>0</td> <td>1</td> <td>0</td> <td>UDP</td>   </tr> <tr>   <td>0</td> <td>1</td> <td>0</td> <td>0</td>
   <td>MACRAW</td>   </tr>
    </table>
...
/**
    ...
    @details @ref Sn_IR indicates the status of Socket Interrupt such as establishment, termination, receiving data,
   timeout).\n When an interrupt occurs and the corresponding bit of @ref Sn_IMR is  the corresponding bit of @ref Sn_IR
   becomes \n In order to clear the @ref Sn_IR bit, the host should write the bit to \n <table> <tr>  <td>7</td>
   <td>6</td> <td>5</td> <td>4</td> <td>3</td> <td>2</td> <td>1</td> <td>0</td>   </tr> <tr>  <td>Reserved</td>
   <td>Reserved</td> <td>Reserved</td> <td>SEND_OK</td> <td>TIMEOUT</td> <td>RECV</td> <td>DISCON</td> <td>CON</td>
   </tr>
    </table>
...
/**
    ...
    - @ref SOCK_SYNRECV    	: It indicates Socket n successfully received the connect-request packet (SYN packet) from a
   peer.
...
/**
    ...
    @details @ref Sn_DHAR configures the destination hardware address of Socket n when using SEND_MAC command in UDP
   mode or it indicates that it is acquired in ARP-process by CONNECT/SEND command.
*/
...
/**
    ...
    @details @ref Sn_DIPR configures or indicates the destination IP address of Socket n. It is valid when Socket n is
   used in TCP/UDP mode. In TCP client mode, it configures an IP address of TCP serverbefore CONNECT command. In TCP
   server mode, it indicates an IP address of TCP clientafter successfully establishing connection. In UDP mode, it
   configures an IP address of peer to be received the UDP packet by SEND or SEND_MAC command.
*/
...
/**
    ...
    @details @ref Sn_DPORT configures or indicates the destination port number of Socket n. It is valid when Socket n is
   used in TCP/UDP mode. In TCP clientmode, it configures the listen port number of TCP serverbefore CONNECT command. In
   TCP Servermode, it indicates the port number of TCP client after successfully establishing connection. In UDP mode,
   it configures the port number of peer to be transmitted the UDP packet by SEND/SEND_MAC command.
*/
...
/**
    ...
    user can re-configure its size using @ref Sn_RXBUF_SIZE. The total sum of @ref Sn_RXBUF_SIZE can not be exceed
   16Kbytes. When exceeded, the data reception error is occurred.
*/
...
/**
    ...
    @details @ref Sn_TXBUF_SIZE configures the TX buffer block size of Socket n. Socket n TX Buffer Block size can be
   configured with 1,2,4,8, and 16 Kbytes. If a different size is configured, the data can�t be normally transmitted to
   a peer. Although Socket n TX Buffer Block size is initially configured to 2Kbytes, user can be re-configure its size
   using @ref Sn_TXBUF_SIZE. The total sum of @ref Sn_TXBUF_SIZE can not be exceed 16Kbytes. When exceeded, the data
...
/**
    ...
    @details @ref Sn_TX_FSR indicates the free size of Socket n TX Buffer Block. It is initialized to the configured
   size by @ref Sn_TXBUF_SIZE. Data bigger than @ref Sn_TX_FSR should not be saved in the Socket n TX Buffer because the
   bigger data overwrites the previous saved data not yet sent. Therefore, check before saving the data to the Socket n
   TX Buffer, and if data is equal or smaller than its checked size, transmit the data with SEND/SEND_MAC command after
   saving the data in Socket n TX buffer. But, if data is bigger than its checked size, transmit the data after dividing
   into the checked size and saving in the Socket n TX buffer.
*/
...
/**
    ...
    @details @ref Sn_TX_RD is initialized by OPEN command. However, if Sn_MR(P[3:0]) is TCP mode(001, it is
   re-initialized while connecting with TCP. After its initialization, it is auto-increased by SEND command. SEND
   command transmits the saved data from the current @ref Sn_TX_RD to the @ref Sn_TX_WR in the Socket n TX Buffer. After
   transmitting the saved data, the SEND command increases the @ref Sn_TX_RD as same as the @ref Sn_TX_WR. If its
   increment value exceeds the maximum value 0xFFFF, (greater than 0x10000 and the carry bit occurs), then the carry bit
   is ignored and will automatically update with the lower 16bits value.
*/
...
/**
    ...
    @details @ref Sn_TX_WR is initialized by OPEN command. However, if Sn_MR(P[3:0]) is TCP mode(001, it is
   re-initialized while connecting with TCP.\n It should be read or be updated like as follows.\n
    1. Read the starting address for saving the transmitting data.\n
    2. Save the transmitting data from the starting address of Socket n TX buffer.\n
    3. After saving the transmitting data, update @ref Sn_TX_WR to the increased value as many as transmitting data
   size. If the increment value exceeds the maximum value 0xFFFF(greater than 0x10000 and the carry bit occurs), then
...
/**
    ...
    Each bit corresponds to each bit of @ref Sn_IR. When a Socket n Interrupt is occurred and the corresponding bit of
   @ref Sn_IMR is the corresponding bit of @ref Sn_IR becomes  When both the corresponding bit of @ref Sn_IMR and @ref
   Sn_IR are and the n-th bit of @ref IR is Host is interrupted by asserted INTn PIN to low.
*/
...
/**
    ...
    @details @ref Sn_KPALVTR configures the transmitting timer of �KEEP ALIVE(KA)packet of SOCKETn. It is valid only in
   TCP mode, and ignored in other modes. The time unit is 5s. KA packet is transmittable after @ref Sn_SR is changed to
   SOCK_ESTABLISHED and after the data is transmitted or received to/from a peer at least once. In case of '@ref
   Sn_KPALVTR > 0', W5500 automatically transmits KA packet after time-period for checking the TCP connection
   (Auto-keepalive-process). In case of '@ref Sn_KPALVTR = 0', Auto-keep-alive-process will not operate, and KA packet
   can be transmitted by SEND_KEEP command by the host (Manual-keep-alive-process). Manual-keep-alive-process is ignored
   in case of '@ref Sn_KPALVTR > 0'.
*/
...
/**
    @brief Reset
    @details If this bit is  All internal registers will be initialized. It will be automatically cleared as after S/W
   reset.
*/
#define MR_RST 0x80
...
/**
    ...
    1 : Enable WOL mode\n
    If WOL mode is enabled and the received magic packet over UDP has been normally processed, the Interrupt PIN (INTn)
   asserts to low. When using WOL mode, the UDP Socket should be opened with any source port number. (Refer to Socket n
   Mode Register (@ref Sn_MR) for opening Socket.)
    @note The magic packet over UDP supported by W5500 consists of 6 bytes synchronization stream (xFFFFFFFFFFFF and
    16 times Target MAC address stream in UDP payload. The options such like password are ignored. You can use any UDP
...
/**
    ...
    When this bit is  Destination Information such as IP address and Port number may be checked with the corresponding
   @ref UIPR & @ref UPORTR.
*/
...
/**
    ...
    To use multicasting, @ref Sn_DIPR & @ref Sn_DPORT should be respectively configured with the multicast group IP
   address & port number before Socket n is opened by OPEN command of @ref Sn_CR.
*/
...
/**
    ...
    But when a TCP clientconnection request is failed, @ref Sn_IR(3) becomes and the status of @ref Sn_SR changes to
   SOCK_CLOSED.
*/
...
/**
    ...
    @details  To connect, a connect-request (SYN packet) is sent to <b>TCP server</b>configured by @ref Sn_DIPR &
   Sn_DPORT(destination address & port). If the connect-request is successful, the @ref Sn_SR is changed to @ref
   SOCK_ESTABLISHED and the Sn_IR(0) becomes \n\n The connect-request fails in the following three cases.\n
    1. When a @b ARPTO occurs (@ref Sn_IR[3] =  ) because destination hardware address is not acquired through the
   ARP-process.\n
    2. When a @b SYN/ACK packet is not received and @b TCPTO (Sn_IR(3) =  )\n
    3. When a @b RST packet is received instead of a @b SYN/ACK packet. In these cases, @ref Sn_SR is changed to @ref
   SOCK_CLOSED.
...
/**
    ...
    @details Regardless of <b>TCP server</b>or <b>TCP client</b> the DISCON command processes the disconnect-process
   (b>Active close</b>or <b>Passive close</b>.\n
    @par Active close
    ...
    @details When the disconnect-process is successful (that is, FIN/ACK packet is received successfully), @ref Sn_SR is
   changed to @ref SOCK_CLOSED.\n Otherwise, TCPTO occurs (\ref Sn_IR(3)='1') and then @ref Sn_SR is changed to @ref
   SOCK_CLOSED.
...
/**
    ...
    Normally SEND transmits data after destination hardware address is acquired by the automatic ARP-process(Address
   Resolution Protocol).\n But SEND_MAC transmits data without the automatic ARP-process.\n In this case, the
   destination hardware address is acquired from @ref Sn_DHAR configured by host, instead of APR-process.
...
/**
    ...
    If the peer can not respond to the keep-alive packet during timeout time, the connection is terminated and the
...
/**
    ...
    @details RECV completes the processing of the received data in Socket n RX Buffer by using a RX read pointer
   register (@ref Sn_RX_RD).\n For more details, refer to Socket n RX Received Size Register (@ref Sn_RX_RSR), Socket n
   RX Write Pointer Register (@ref Sn_RX_WR), and Socket n RX Read Pointer Register (@ref Sn_RX_RD).
*/
...
/**
    @brief CON Interrupt
    @details This is issued one time when the connection with peer is successful and then @ref Sn_SR is changed to @ref
   SOCK_ESTABLISHED.
*/
...
/**
    ...
    When DICON, CLOSE command is ordered, or when a timeout occurs, it is changed to @ref SOCK_CLOSED regardless of
   previous status.
*/
...
/**
    @brief Listen state
    @details This indicates Socket n is operating as <b>TCP server</b>mode and waiting for connection-request (SYN
   packet) from a peer <b>TCP client</b>.\n It will change to @ref SOCK_ESTALBLISHED when the connection-request is
   successfully accepted.\n Otherwise it will change to @ref SOCK_CLOSED after TCPTO @ref Sn_IR(TIMEOUT) = '1') is
   occurred.
*/
...
/**
    ...
    It is temporarily shown when @ref Sn_SR is changed from @ref SOCK_INIT to @ref SOCK_ESTABLISHED by CONNECT
   command.\n If connect-accept(SYN/ACK packet) is received from the peer at SOCK_SYNSENT, it changes to @ref
...
/**
    ...
    It changes to @ref SOCK_ESTABLISHED when the <b>TCP SERVER</b>processed the SYN packet from the <b>TCP
   CLIENT</b>during @ref SOCK_LISTEN, or when the CONNECT command is successful.\n During @ref SOCK_ESTABLISHED, DATA
   packet can be transferred using SEND or RECV command.
*/
...
/**
    @brief Closing state
    @details This indicates Socket n is waiting for the response (FIN/ACK packet) to the disconnect-request (FIN packet)
   by passive-close.\n It changes to @ref SOCK_CLOSED when Socket n received the response successfully, or when
