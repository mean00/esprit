...
set(COR_IS_MULTI_CONFIG
    "${COR_IS_MULTI_CONFIG}"
    CACHE BOOL "Do not change this" FORCE)
...
option(CORROSION_NO_WARN_PARSE_TARGET_TRIPLE_FAILED "Surpresses a warning if the parsing the target triple failed." OFF)
...
if(CMAKE_HOST_SYSTEM_NAME STREQUAL "Darwin" AND CMAKE_SYSTEM_NAME STREQUAL "iOS")
  ...
  set(CORROSION_HOST_TARGET_LINKER
      "${_corrosion_host_linker}"
      CACHE
        STRING
...
if(CMAKE_GENERATOR MATCHES "Visual Studio"
  ...
  message(
    FATAL_ERROR
      "Due to a cargo issue, cross-compiling with a Visual Studio generator and rust versions"
...
if(Rust_TOOLCHAIN_IS_RUSTUP_MANAGED AND DEFINED Rust_RUSTUP_TOOLCHAINS)
  set(corrosion_tools_rust_toolchain_docstring
      "Rust toolchain to use for building helper tools such as cbindgen or cxx-bridge")
  ...
  set(CORROSION_TOOLS_RUST_TOOLCHAIN
      "${cor_default_tools_toolchain}"
      CACHE STRING "${corrosion_tools_rust_toolchain_docstring}" FORCE)
  ...
  if(NOT "$CACHE{CORROSION_TOOLS_RUST_TOOLCHAIN}" IN_LIST Rust_RUSTUP_TOOLCHAINS)
    if("$CACHE{CORROSION_TOOLS_RUST_TOOLCHAIN}-${Rust_CARGO_HOST_TARGET}" IN_LIST Rust_RUSTUP_TOOLCHAINS)
      set(CORROSION_TOOLS_RUST_TOOLCHAIN
          "$CACHE{CORROSION_TOOLS_RUST_TOOLCHAIN}-${Rust_CARGO_HOST_TARGET}"
  ...
  foreach(
    ...
    if("${toolchain}" STREQUAL $CACHE{CORROSION_TOOLS_RUST_TOOLCHAIN})
      ...
      if(NOT (tc_cargo AND tc_rustc))
        message(
          FATAL_ERROR "Failed to find executable rustc or cargo for toolchain `$CACHE{CORROSION_TOOLS_RUST_TOOLCHAIN}`")
      endif()
      set(CORROSION_TOOLS_RUSTC
          "${tc_rustc}"
          CACHE INTERNAL "" FORCE)
      set(CORROSION_TOOLS_CARGO
          "${tc_cargo}"
          CACHE INTERNAL "" FORCE)
      break()
    endif()
...
function(_corrosion_bin_target_suffix target_name out_var_suffix)
  ...
  if((hostbuild AND CMAKE_HOST_WIN32) OR ((NOT hostbuild) AND (Rust_CARGO_TARGET_OS STREQUAL "windows")))
    set(_suffix ".exe")
...
function(_handle_output_directory_genex input_path config_type output_path)
  if("${config_type}" STREQUAL "")
    # Prevent new path from being `dir//file`, since that causes issues with the file dependency.
  ...
  if("${stripped_out_dir}" STREQUAL "${curr_out_dir}")
    ...
    message(WARNING "Encountered output directory path with unsupported genex. " "Output dir: `${curr_out_dir}`"
...
#
# This function should be called deferred to evaluate target properties late in the configure stage. IMPORTED_LOCATION
# does not support Generator expressions, so we must evaluate the output directory target property value at configure
# time. This function must be deferred to the end of the configure stage, so we can be sure that the output directory is
...
function(_corrosion_set_imported_location_deferred target_name base_property output_directory_property filename)
  # The output directory property is expected to be set on the exposed target (without postfix), but we need to set the
  # imported location on the actual library target with postfix.
  ...
  # Append .exe suffix for executable by-products if the target is windows or if it's a host build and the host is
  # Windows.
  ...
  message(
    DEBUG
    "Output directory property (target ${output_dir_prop_target_name}): ${output_directory_property} dir: ${output_directory}"
  )
  ...
  foreach(config_type ${CMAKE_CONFIGURATION_TYPES})
    ...
    if(NOT DEFINED sanitized_out_dir)
      message(
        FATAL_ERROR
          "${output_directory_property} for target ${output_dir_prop_target_name} "
    ...
    set_property(TARGET ${target_name} PROPERTY "${base_property}_${config_type_upper}"
  ...
  if(NOT COR_IS_MULTI_CONFIG)
    ...
    if(NOT DEFINED sanitized_output_directory)
      message(
        FATAL_ERROR
          "${output_dir_prop_target_name} for target ${output_dir_prop_target_name} "
  ...
  message(DEBUG "Setting ${base_property} for target ${target_name}" " to `${base_output_directory}/${filename}`.")

  # IMPORTED_LOCATION must be set regardless of possible overrides. In the multiconfig case, the last configuration
  # "wins" (IMPORTED_LOCATION is not documented to have Genex support).
  set_property(TARGET ${target_name} PROPERTY "${base_property}" "${base_output_directory}/${filename}")
endfunction()

# Set the imported location of a Rust target.
#
# Rust targets are built via custom targets / custom commands. The actual artifacts are exposed to CMake as imported
# libraries / executables that depend on the cargo_build command. For CMake to find the built artifact we need to set
# the IMPORTED location to the actual location on disk. Corrosion tries to copy the artifacts built by cargo to standard
# locations. The IMPORTED_LOCATION is set to point to the copy, and not the original from the cargo build directory.
#
# Parameters: - target_name: Name of the Rust target - base_property: Name of the base property - i.e.
# `IMPORTED_LOCATION` or `IMPORTED_IMPLIB`. - output_directory_property: Target property name that determines the
# standard location for the artifact. - filename of the artifact.
...
function(_corrosion_copy_byproduct_deferred target_name output_dir_prop_names cargo_build_dir file_names)
  ...
  foreach(config_type ${CMAKE_CONFIGURATION_TYPES})
    ...
    elseif(output_dir)
      ...
      # Only add config dir if there is no genex in here. See
      # https://cmake.org/cmake/help/latest/prop_tgt/RUNTIME_OUTPUT_DIRECTORY.html Logic duplicated from
      # _corrosion_set_imported_location_deferred
      ...
      # Fallback to the default directory. We do not append the configuration directory here and instead let CMake do
      # this, since otherwise the resolving of dynamic library imported paths may fail.
      set(curr_out_dir "${CMAKE_CURRENT_BINARY_DIR}")
    endif()
...
#
# Parameters: - target_name: The name of the Rust target - output_dir_prop_names: The property name(s) controlling the
# destination (e.g. `LIBRARY_OUTPUT_DIRECTORY` or `PDB_OUTPUT_DIRECTORY;RUNTIME_OUTPUT_DIRECTORY`) - cargo_build_dir:
# the directory cargo build places it's output artifacts in. - filenames: the file names of any output artifacts as a
# list.
...
# Add targets for the static and/or shared libraries of the rust target. The generated byproduct names are returned via
# the `OUT_<type>_BYPRODUCTS` arguments.
function(_corrosion_add_library_target)
  set(OPTIONS "")
  set(ONE_VALUE_KEYWORDS WORKSPACE_MANIFEST_PATH TARGET_NAME OUT_ARCHIVE_OUTPUT_BYPRODUCTS OUT_SHARED_LIB_BYPRODUCTS
                         OUT_PDB_BYPRODUCT)
  ...
  if(has_cdylib)
    set("${CALT_OUT_SHARED_LIB_BYPRODUCTS}"
        "${dynamic_lib_name}"
        PARENT_SCOPE)
    if(is_windows)
    ...
    if(is_windows_msvc)
      set("${CALT_OUT_PDB_BYPRODUCT}"
          "${pdb_name}"
          PARENT_SCOPE)
    endif()
  endif()
  set("${CALT_OUT_ARCHIVE_OUTPUT_BYPRODUCTS}"
      "${archive_output_byproducts}"
      PARENT_SCOPE)
  ...
  if(has_staticlib)
    ...
    _corrosion_set_imported_location("${target_name}-static" "IMPORTED_LOCATION" "ARCHIVE_OUTPUT_DIRECTORY"
                                     "${static_lib_name}")

    # Todo: NO_STD target property?
    if(NOT COR_NO_STD)
      set_property(TARGET ${target_name}-static PROPERTY INTERFACE_LINK_LIBRARIES ${Rust_CARGO_TARGET_LINK_NATIVE_LIBS})
      set_property(TARGET ${target_name}-static PROPERTY INTERFACE_LINK_OPTIONS ${Rust_CARGO_TARGET_LINK_OPTIONS})
      if(is_macos)
  ...
  if(has_cdylib)
    ...
    _corrosion_set_imported_location("${target_name}-shared" "IMPORTED_LOCATION" "LIBRARY_OUTPUT_DIRECTORY"
                                     "${dynamic_lib_name}")
    # In the future we would probably prefer to let Rust set the soname for packages >= 1.0. This is tracked in issue
    # #333.
    set_target_properties(${target_name}-shared PROPERTIES IMPORTED_NO_SONAME TRUE)

    if(is_windows)
      _corrosion_set_imported_location("${target_name}-shared" "IMPORTED_IMPLIB" "ARCHIVE_OUTPUT_DIRECTORY"
                                       "${implib_name}")
      set_target_properties(${target_name}-shared PROPERTIES COR_IMPLIB_FILE_NAME ${implib_name})
    endif()

    if(is_macos)
      set_property(TARGET ${target_name}-shared PROPERTY INTERFACE_LINK_DIRECTORIES
...
function(_corrosion_add_bin_target workspace_manifest_path bin_name out_bin_byproduct out_pdb_byproduct)
  ...
  if(Rust_CARGO_TARGET_ENV STREQUAL "msvc")
    set(${out_pdb_byproduct}
        "${pdb_name}"
        PARENT_SCOPE)
  endif()

  # Potential .exe suffix will be added later, also depending on possible hostbuild target property
  set(bin_filename "${bin_name}")
  set(${out_bin_byproduct}
      "${bin_filename}"
      PARENT_SCOPE)
  add_dependencies(${bin_name} cargo-build_${bin_name})

  if(Rust_CARGO_TARGET_OS STREQUAL "darwin")
    set_property(TARGET ${bin_name} PROPERTY INTERFACE_LINK_DIRECTORIES
                                             "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/lib")
  endif()

  _corrosion_set_imported_location("${bin_name}" "IMPORTED_LOCATION" "RUNTIME_OUTPUT_DIRECTORY" "${bin_filename}")

endfunction()

include(CorrosionGenerator)

# Note: `cmake_language(GET_MESSAGE_LOG_LEVEL <output_variable>)` requires CMake 3.25, so we offer our own option to
# control verbosity of downstream commands (e.g. cargo build)
if(CORROSION_VERBOSE_OUTPUT)
  set(_CORROSION_VERBOSE_OUTPUT_FLAG
      --verbose
      CACHE INTERNAL "")
else()
  # We want to silence some less important commands by default.
  set(_CORROSION_QUIET_OUTPUT_FLAG
      --quiet
      CACHE INTERNAL "")
endif()

set(_CORROSION_CARGO_VERSION
    ${Rust_CARGO_VERSION}
    CACHE INTERNAL "cargo version used by corrosion")
set(_CORROSION_RUST_CARGO_TARGET
    ${Rust_CARGO_TARGET}
    CACHE INTERNAL "target triple used by corrosion")
set(_CORROSION_RUST_CARGO_HOST_TARGET
    ${Rust_CARGO_HOST_TARGET}
    CACHE INTERNAL "host triple used by corrosion")
set(_CORROSION_RUSTC
    "${RUSTC_EXECUTABLE}"
    CACHE INTERNAL "Path to rustc used by corrosion")
set(_CORROSION_CARGO
    "${CARGO_EXECUTABLE}"
    CACHE INTERNAL "Path to cargo used by corrosion")

string(REPLACE "-" "_" _CORROSION_RUST_CARGO_TARGET_UNDERSCORE "${Rust_CARGO_TARGET}")
set(_CORROSION_RUST_CARGO_TARGET_UNDERSCORE
    "${_CORROSION_RUST_CARGO_TARGET_UNDERSCORE}"
    CACHE INTERNAL "lowercase target triple with underscores")
...
set(_CORROSION_RUST_CARGO_TARGET_UPPER
    ...
    CACHE INTERNAL "target triple in uppercase with underscore")
...
# immediately, we are using a different property name depending on the CMake version. However users avoid using any of
# the properties directly, as they are no longer part of the public API and are to be considered deprecated. Instead use
# the corrosion_set_... functions as documented in the Readme.
set(_CORR_PROP_FEATURES
    CORROSION_FEATURES
    CACHE INTERNAL "")
set(_CORR_PROP_ALL_FEATURES
    CORROSION_ALL_FEATURES
    CACHE INTERNAL "")
set(_CORR_PROP_NO_DEFAULT_FEATURES
    CORROSION_NO_DEFAULT_FEATURES
    CACHE INTERNAL "")
set(_CORR_PROP_ENV_VARS
    CORROSION_ENVIRONMENT_VARIABLES
    CACHE INTERNAL "")
set(_CORR_PROP_HOST_BUILD
    CORROSION_USE_HOST_BUILD
    CACHE INTERNAL "")
...
function(_add_cargo_build out_cargo_build_out_dir)
  ...
  cmake_parse_arguments(ACB "${options}" "${one_value_args}" "${multi_value_args}" ${ARGN})
  ...
  elseif("staticlib" IN_LIST target_kinds OR "cdylib" IN_LIST target_kinds)
    ...
    if("${Rust_VERSION}" VERSION_GREATER_EQUAL "1.64")
      # https://doc.rust-lang.org/1.64.0/cargo/commands/cargo-rustc.html `--crate-type` is documented since Rust 1.64
      # for `cargo rustc`. We just unconditionally set it when available, to support overriding the crate type. Due to
      # https://github.com/rust-lang/cargo/issues/14498 we can't use one argument and pass a comma seperated list.
      # Instead we use multiple arguments.
  ...
  # If a CMake sysroot is specified, forward it to the linker rustc invokes, too. CMAKE_SYSROOT is documented to be
  # passed via --sysroot, so we assume that when it's set, the linker supports this option in that style.
  ...
  set(global_rustflags_target_property
      "$<TARGET_GENEX_EVAL:${target_name},$<TARGET_PROPERTY:${target_name},INTERFACE_CORROSION_RUSTFLAGS>>")
  set(local_rustflags_target_property
      "$<TARGET_GENEX_EVAL:${target_name},$<TARGET_PROPERTY:${target_name},INTERFACE_CORROSION_LOCAL_RUSTFLAGS>>")
  ...
  set(no_default_features_target_property
      "$<GENEX_EVAL:$<TARGET_PROPERTY:${target_name},${_CORR_PROP_NO_DEFAULT_FEATURES}>>")
  ...
  # We always set `--target`, so that cargo always places artifacts into a directory with the target triple.
  set(cargo_target_option
      "--target=$<IF:${hostbuild_override},${_CORROSION_RUST_CARGO_HOST_TARGET},${_CORROSION_RUST_CARGO_TARGET}>")
  ...
  set(cargo_profile_target_property
      "$<TARGET_GENEX_EVAL:${target_name},$<TARGET_PROPERTY:${target_name},INTERFACE_CORROSION_CARGO_PROFILE>>")
  ...
  # Rust will add `-lSystem` as a flag for the linker on macOS. Adding the -L flag via RUSTFLAGS only fixes the problem
  # partially - buildscripts still break, since they won't receive the RUSTFLAGS. This seems to only be a problem if we
  # specify the linker ourselves (which we do, since this is necessary for e.g. linking C++ code). We can however set
  # `LIBRARY_PATH`, which is propagated to the build-script-build properly.
  ...
  elseif(CMAKE_CROSSCOMPILING AND CMAKE_HOST_SYSTEM_NAME STREQUAL "Darwin")
    if(${CMAKE_HOST_SYSTEM_VERSION} VERSION_LESS 22)
      set(cargo_library_path
          "$<${hostbuild_override}:LIBRARY_PATH=/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/lib>")
    endif()
  endif()

  set(cargo_profile_set "$<BOOL:${cargo_profile_target_property}>")
  # In the default case just specify --release or nothing to stay compatible with older rust versions.
  ...
  set(custom_profile_build_type_dir
      "$<IF:${profile_dir_is_overridden},${profile_dir_override},${cargo_profile_target_property}>")
  ...
  if(CMAKE_C_COMPILER)
    # This variable is read by cc-rs (often used in build scripts) to determine the c-compiler. It can still be
    # overridden if the user sets the non underscore variant via the environment variables on the target.
  ...
  # cc-rs doesn't seem to support `llvm-ar` (commandline syntax), wo we might as well just use the default AR.
  ...
  # When using XCode to target iOS / iOSSimulator, `cc` will be a compiler that targets iOS. (Presumably this is because
  # XCode modifies PATH). This causes linker errors, because Rust compiles build-scripts and proc-macros for the
  # host-platform, and assumes `cc` is a valid linker driver for the host platform (but in this case `cc` targets iOS).
  ...
  # Since we instruct cc-rs to use the compiler found by CMake, it is likely one that requires also specifying the
  # target sysroot to use. CMake's generator makes sure to pass --sysroot with CMAKE_OSX_SYSROOT. Fortunately the
  # compilers Apple ships also respect the SDKROOT environment variable, which we can set for use when cc-rs invokes the
  # compiler.
  ...
  corrosion_add_target_local_rustflags("${target_name}"
                                       "$<$<BOOL:${corrosion_link_args}>:-Clink-args=${corrosion_link_args}>")
  ...
  # With the MSVC ABI rustc only supports directly invoking the linker - Invoking cl as the linker driver is not
  # supported.
  ...
  if(Rust_CROSSCOMPILING AND (CMAKE_C_COMPILER_TARGET OR CMAKE_CXX_COMPILER_TARGET))
    ...
    # Skip adding the linker argument, if the linker is explicitly set, since the explicit_linker_property will not be
    # set when this function runs. Passing this rustflag is necessary for clang.
  ...
  add_custom_target(
    ...
    # Build crate
    COMMAND
      ${CMAKE_COMMAND} -E env "${build_env_variable_genex}" "${global_rustflags_genex}" "${cargo_target_linker}"
      "${cargo_host_target_linker}" "${corrosion_cc_rs_flags}" "${cargo_library_path}"
      "CORROSION_BUILD_DIR=${CMAKE_CURRENT_BINARY_DIR}" "CARGO_BUILD_RUSTC=${rustc_bin}" "${cargo_bin}" rustc
      ${cargo_rustc_filter} ${cargo_target_option} ${_CORROSION_VERBOSE_OUTPUT_FLAG} ${all_features_arg}
      ${no_default_features_arg} ${features_genex} --package ${package_name} ${cargo_rustc_crate_types} --manifest-path
      "${path_to_toml}" --target-dir "${cargo_target_dir}" ${cargo_profile} ${flags_genex}
      # Any arguments to cargo must be placed before this line
      ${local_rustflags_delimiter} ${local_rustflags_genex}
  ...
  # User exposed custom target, that depends on the internal target. Corrosion post build steps are added on the
  # internal target, which ensures that they run before any user defined post build steps on this target.
  add_custom_target(cargo-build_${target_name} ALL)
  add_dependencies(cargo-build_${target_name} _cargo-build_${target_name})

  # Add custom target before actual build that user defined custom commands (e.g. code generators) can use as a hook to
  # do something before the build. This mainly exists to not expose the `_cargo-build` targets.
  ...
  add_custom_target(
    ...
    COMMAND "${cargo_bin}" clean ${cargo_target_option} -p ${package_name} --manifest-path "${path_to_toml}"
    # Set WORKING_DIRECTORY to the directory containing the manifest, so that configuration files
    # such as `.cargo/config.toml` or `toolchain.toml` are applied as expected. Cargo searches for
    # configuration files by walking upward from the current directory.
    WORKING_DIRECTORY "${workspace_toml_dir}"
...
function(corrosion_import_crate)
  ...
  if(COR_PROFILE)
    if(Rust_VERSION VERSION_LESS 1.57.0)
      message(FATAL_ERROR "Selecting custom profiles via `PROFILE` requires at least rust 1.57.0, but you "
                          "have ${Rust_VERSION}.")
      # The profile name could be part of a Generator expression, so this won't catch all occurences. Since it is hard
      # to add an error message for genex, we don't do that here.
    elseif("${COR_PROFILE}" STREQUAL "test" OR "${COR_PROFILE}" STREQUAL "bench")
      message(
        FATAL_ERROR
          "Corrosion does not support building Rust crates with the cargo profiles"
  ...
  # intended to be used with foreach(... ZIP_LISTS ...), meaning that the crate_types at index i of
  # `override_crate_type_types_list` are for the package_name at index i of `override_crate_type_package_name_list`. It
  # would really be nice if CMake had structs or dicts.
  ...
  if(DEFINED COR_OVERRIDE_CRATE_TYPE)
    string(
      JOIN " " usage_help "Each argument to OVERRIDE_CRATE_TYPE must be of the form `<package_name>=<crate_type(s)>."
    ...
    elseif(NOT num_override_packages EQUAL num_override_packages2)
      ...
      set(override_crate_types_arg "OVERRIDE_CRATE_TYPE_ARGS" "override_crate_type_package_name_list"
                                   "override_crate_type_types_list")
    endif()
  endif()
  ...
  # Not target props yet: NO_STD NO_LINKER_OVERRIDE # We could simply zero INTERFACE_CORROSION_LINKER if this is set.
  ...
  set_target_properties(
    ${imported_crates}
    PROPERTIES "${_CORR_PROP_ALL_FEATURES}" "${COR_ALL_FEATURES}"
...
function(corrosion_set_linker target_name linker)
  ...
  set_property(TARGET ${target_name} PROPERTY INTERFACE_CORROSION_LINKER "${linker}")
endfunction()
...
function(corrosion_set_hostbuild target_name)
  ...
  set_property(TARGET ${target_name} PROPERTY ${_CORR_PROP_HOST_BUILD} 1)
endfunction()

# Add flags for rustc (RUSTFLAGS) which affect the target and all of it's Rust dependencies
#
# Additional rustflags may be passed as optional parameters after rustflag. Please note, that if you import multiple
# targets from a package or workspace, but set different Rustflags via this function, the Rust dependencies will have to
# be rebuilt when changing targets. Consider `corrosion_add_target_local_rustflags()` as an alternative to avoid this.
...
function(corrosion_set_features target_name)
  ...
  cmake_parse_arguments(PARSE_ARGV 1 SET "${options}" "${one_value_args}" "${multi_value_args}")

  if(DEFINED SET_ALL_FEATURES)
    set_property(TARGET ${target_name} PROPERTY ${_CORR_PROP_ALL_FEATURES} ${SET_ALL_FEATURES})
  endif()
  if(SET_NO_DEFAULT_FEATURES)
    set_property(TARGET ${target_name} PROPERTY ${_CORR_PROP_NO_DEFAULT_FEATURES} 1)
  endif()
  if(SET_FEATURES)
    set_property(
      TARGET ${target_name}
...
function(corrosion_link_libraries target_name)
  ...
  foreach(library ${ARGN})
    set_property(
      TARGET _cargo-build_${target_name}
      APPEND
      PROPERTY CARGO_DEPS_LINKER_LANGUAGES $<TARGET_PROPERTY:${library},LINKER_LANGUAGE>)

    if(TARGET "${library}")
      # This works fine, except when compiling for ios. See https://cmake.org/pipermail/cmake/2016-March/063050.html
      # XCODE_EMIT_EFFECTIVE_PLATFORM_NAME=OFF is supposed to prevent emitting EFFECTIVE_PLATFORM_NAME, but even with
      # CMake 4.1 and the variable set to off EFFECTIVE_PLATFORM_NAME still leaks into generator expressions, and is not
      # correctly replaced at build time
      ...
      corrosion_add_target_local_rustflags(${target_name} "-L${linker_dir}"
                                           "-l$<TARGET_LINKER_FILE_BASE_NAME:${library}>")
      add_dependencies(_cargo-build_${target_name} ${library})
    elseif(IS_ABSOLUTE "${library}")
      # Linking via full path (See
      # https://doc.rust-lang.org/rustc/command-line-arguments.html#linking-modifiers-verbatim)
...
function(corrosion_install)
  ...
  # The different install types that are supported. Some targets may have more than one of these types. For example, on
  # Windows, a shared library will have both an ARCHIVE component and a RUNTIME component.
  ...
  if(INSTALL_TYPE STREQUAL "TARGETS")
    ...
    if(ARGN_LENGTH)
      ...
      if(FRONT STREQUAL "EXPORT")
        ...
        # Remove first, since otherwise we will append to the file on every reconfigure. Assumes that the
        # corrosion_install will only be called once for a given EXPORT_NAME.
    ...
    list(LENGTH ARGN ARGN_LENGTH)
    while(ARGN_LENGTH)
      # Check if we're dealing with arguments for a specific install target type, or with default options for all target
      # types.
      list(GET ARGN 0 FRONT)
      ...
      list(LENGTH ARGN ARGN_LENGTH)
      while(ARGN_LENGTH)
        # If the next keyword is an install target type, then break - arguments have been gathered.
        list(GET ARGN 0 FRONT)
        if(FRONT IN_LIST INSTALL_TARGET_TYPES)
          break()
        endif()
      ...
      cmake_parse_arguments(COR "${OPTIONS}" "${ONE_VALUE_ARGS}" "${MULTI_VALUE_ARGS}" ${ARGS})
    ...
    foreach(INSTALL_TARGET ${INSTALL_TARGETS})
      ...
      # Don't both implementing target type differentiation using generator expressions since TYPE cannot change after
      # target creation
      get_property(
        TARGET_TYPE
      ...
      if(TARGET_TYPE STREQUAL "EXECUTABLE")
        ...
        elseif(DEFINED COR_INSTALL_DEFAULT_PERMISSIONS)
          ...
          set(PERMISSIONS ${DEFAULT_PERMISSIONS} OWNER_EXECUTE GROUP_EXECUTE WORLD_EXECUTE)
        endif()
          ...
          if(NOT PUBLIC_HEADER MATCHES .*-NOTFOUND)
            foreach(INCLUDE_DIRECTORY ${PUBLIC_HEADER})
              install(
                ...
                DIRECTORY_PERMISSIONS ${PERMISSIONS} ${CONFIGURATIONS})
            endforeach()
          endif()
        endforeach()
      else()
        install(
          TARGETS ${INSTALL_TARGET} ${EXPORT_NAME} FILE_SET HEADERS
          DESTINATION ${DESTINATION}
          PERMISSIONS ${PERMISSIONS} ${CONFIGURATIONS})
      endif()
    endforeach()
...
function(_corrosion_check_cxx_version_helper manifest_dir cxx_name out_required_version)
  execute_process(
    COMMAND
      ${CMAKE_COMMAND} -E env "CARGO_BUILD_RUSTC=${_CORROSION_RUSTC}" ${_CORROSION_CARGO} tree -i "${cxx_name}"
      # Usage of `cxx` could be gated behind a feature. Features can use Generator expressions, so we can't really know
      # what features we will enable when building at this point. Features should be additive though, so simply enabling
      # all-features should work for dependency resolution.
      --all-features --target all --depth=0
...
function(_corrosion_check_cxx_version manifest_dir out_required_version)
  # cxxbridge-cmd is known to be available in lockfiles since cxx 1.0.131. We include `cxx` as a fallback to support
  # older versions too. `cxxbridge` should always be exactly the same version as `cxx`, so falling back to `cxx` version
  # should not cause issues.
  foreach(cxxbridge_name cxxbridge-cmd cxx)
    unset(cxx_required_version)
    _corrosion_check_cxx_version_helper("${manifest_dir}" "${cxxbridge_name}" cxx_required_version)
...
function(corrosion_add_cxxbridge cxx_target)
  ...
  if(NOT cxx_required_version)
    message(FATAL_ERROR "Failed to find a dependency on `cxxbridge-cmd` / `cxx` for crate ${_arg_CRATE}")
  endif()
  ...
  if(NOT cxxbridge)
    if(NOT TARGET "cxxbridge_v${cxx_required_version}")
      ...
      add_custom_command(
        OUTPUT "${CMAKE_BINARY_DIR}/corrosion/cxxbridge_v${cxx_required_version}/bin/cxxbridge${executable_postfix}"
        COMMAND ${CMAKE_COMMAND} -E make_directory "${CMAKE_BINARY_DIR}/corrosion/cxxbridge_v${cxx_required_version}"
        COMMAND
          ${CMAKE_COMMAND} -E env "CARGO_BUILD_RUSTC=$CACHE{CORROSION_TOOLS_RUSTC}" $CACHE{CORROSION_TOOLS_CARGO}
          install cxxbridge-cmd --version "${cxx_required_version}" --locked --root
          "${CMAKE_BINARY_DIR}/corrosion/cxxbridge_v${cxx_required_version}" --quiet
          # todo: use --target-dir to potentially reuse artifacts
        COMMENT
          "Building cxxbridge (version ${cxx_required_version}) with Rust toolchain $CACHE{CORROSION_TOOLS_RUST_TOOLCHAIN}"
      )
      add_custom_target(
  ...
  #
  # CMAKE_CURRENT_BINARY_DIR corrosion_generated cxxbridge <cxx_target> include <cxx_target> <headers> rust cxx.h src
  # <sourcefiles> cbindgen ... other ...
  ...
  target_include_directories(${cxx_target} PUBLIC $<BUILD_INTERFACE:${generated_dir}/include>
                                                  $<INSTALL_INTERFACE:include>)

  # cxx generated code is using c++11 features in headers, so propagate c++11 as minimal requirement message(STATUS
  # "Target ::: ${cxx_target}") target_compile_features(${cxx_target} PUBLIC cxx_std_11)
  ...
  add_custom_command(
    ...
    COMMAND ${cxxbridge} --header --output "${generated_dir}/include/rust/cxx.h"
  ...
  foreach(filepath ${_arg_FILES})
    ...
    add_custom_command(
      OUTPUT "${header_placement_dir}/${cxx_header}" "${source_placement_dir}/${cxx_source}"
      COMMAND ${cxxbridge} ${rust_source_path} --header --output "${header_placement_dir}/${cxx_header}"
      COMMAND ${cxxbridge} ${rust_source_path} --output "${source_placement_dir}/${cxx_source}" --include
              "${cxx_target}/${cxx_header}"
  ...
  # Make sure to export the headers with PUBLIC. This ensures that any target that depends on cxx_target also has these
  # files as a dependency CMake will then make sure to generate the files before building either target, which is
  # important in the presence of circular dependencies
  target_sources(${cxx_target} PUBLIC ${GENERATED_HEADERS})

  if(DEFINED _arg_REGEN_TARGET)
    # Add only the headers to the regen target, as the sources are actually not needed For the IDE to pick everything up
    add_custom_target(
      ${_arg_REGEN_TARGET}
...
function(corrosion_experimental_cbindgen)
  ...
  if(NOT
    ...
    message(
      FATAL_ERROR
        "Unknown signature for corrosion_experimental_cbindgen.\n"
  ...
  if(TARGET "${CCN_TARGET}")
    ...
    # todo: as an optimization we could cache the cargo metadata output (but --no-deps makes that slightly more
    # complicated)
  else()
    if(NOT DEFINED CCN_MANIFEST_DIRECTORY)
      message(FATAL_ERROR "Internal error: There should have been a fatal error already if neither TARGET or "
  ...
  if(installed_cbindgen)
    ...
    if(NOT TARGET "_corrosion_cbindgen")
      ...
      add_custom_command(
        OUTPUT "${cbindgen}"
        COMMAND ${CMAKE_COMMAND} -E env "CARGO_BUILD_RUSTC=$CACHE{CORROSION_TOOLS_RUSTC}" $CACHE{CORROSION_TOOLS_CARGO}
                install cbindgen --locked --root "${local_cbindgen_install_dir}" ${_CORROSION_QUIET_OUTPUT_FLAG}
        COMMENT "Building cbindgen with Rust toolchain $CACHE{CORROSION_TOOLS_RUST_TOOLCHAIN}"
        VERBATIM)
      add_custom_target("_corrosion_cbindgen" DEPENDS "${cbindgen}")
    endif()
  endif()
  ...
  if(CMAKE_VERSION VERSION_GREATER_EQUAL "3.23")
    target_sources(${cbindgen_bindings_target} INTERFACE FILE_SET HEADERS BASE_DIRS "${header_placement_dir}" FILES
                                                         "${header_placement_dir}/${output_header_name}")
  else()
    # Note: not clear to me how install would best work before CMake 3.23
    target_include_directories(${cbindgen_bindings_target} INTERFACE $<BUILD_INTERFACE:${header_placement_dir}>
                                                                     $<INSTALL_INTERFACE:include>)
  endif()

  # This may be different from $header_placement_dir since the user specified HEADER_NAME may contain relative
  # directories.
  ...
  add_custom_command(
    ...
    COMMAND
      "${CMAKE_COMMAND}" -E env TARGET="${cbindgen_target_triple}"
      # cbindgen invokes cargo-metadata and checks the CARGO environment variable
      CARGO="${_CORROSION_CARGO}" RUSTC="${_CORROSION_RUSTC}" "${cbindgen}" --output "${generated_header}" --crate
      "${rust_cargo_package}" ${depfile_cbindgen_arg} ${CCN_FLAGS}
  ...
  add_custom_target(
    "_corrosion_cbindgen_${cbindgen_bindings_target}_bindings_${header_identifier}"
    DEPENDS "${generated_header}"
    COMMENT "Generate ${generated_header} for ${cbindgen_bindings_target}")
  add_dependencies("_corrosion_cbindgen_${cbindgen_bindings_target}_bindings"
                   "_corrosion_cbindgen_${cbindgen_bindings_target}_bindings_${header_identifier}")
...
function(corrosion_parse_package_version package_manifest_path out_package_version)
  ...
  # Find the package table. It may contain arrays, so match until \n\[, which should mark the next table. Note:
  # backslashes must be doubled to escape the backslash for the bracket. LF is single backslash however. On windows the
  # line also ends in \n, so matching against \n\[ is sufficient to detect an opening bracket on a new line.
  ...
  if(CMAKE_MATCH_COUNT EQUAL "1")
    ...
    message(DEBUG "Failed to find `[package]` table in package manifest `${package_manifest_path}`.\n"
  ...
  foreach(output_var RUNTIME_OUTPUT_DIRECTORY ARCHIVE_OUTPUT_DIRECTORY LIBRARY_OUTPUT_DIRECTORY PDB_OUTPUT_DIRECTORY)
    ...
    foreach(config_type ${CMAKE_CONFIGURATION_TYPES})
      ...
      if(DEFINED "CMAKE_${output_var}_${config_type_upper}")
        set_property(TARGET ${target_name} PROPERTY "${output_var}_${config_type_upper}"
...
# Helper macro to pass through an optional `OPTION` argument parsed via `cmake_parse_arguments` to another function that
# takes the same OPTION. If the option was set, then the variable <var_name> will be set to the same option name again,
...
# Helper macro to pass through an optional argument with value(s), parsed via `cmake_parse_arguments`, to another
# function that takes the same keyword + associated values. If the argument was given, then the variable <var_name> will
# be a list of the argument name and the values, which will be expanded, when calling the function (assuming no quotes).
