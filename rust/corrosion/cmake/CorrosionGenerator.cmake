function(_cargo_metadata out manifest)
  ...
  execute_process(
    COMMAND
      ${CMAKE_COMMAND} -E env "CARGO_BUILD_RUSTC=${_CORROSION_RUSTC}" "${_CORROSION_CARGO}" metadata --manifest-path
      "${manifest}" --format-version 1
      # We don't care about non-workspace dependencies
      --no-deps ${cargo_locked} ${cargo_frozen}
    # Set WORKING_DIRECTORY to the directory containing the manifest, so that configuration files such as
    # `.cargo/config.toml` or `toolchain.toml` are applied as expected. Cargo searches for configuration files by
    # walking upward from the current directory.
    WORKING_DIRECTORY "${workspace_toml_dir}"
    OUTPUT_VARIABLE json COMMAND_ERROR_IS_FATAL ANY)
...
function(_generator_add_package_targets)
  ...
  set(ONE_VALUE_KEYWORDS WORKSPACE_MANIFEST_PATH PACKAGE_MANIFEST_PATH PACKAGE_NAME PACKAGE_VERSION TARGETS_JSON
                         OUT_CREATED_TARGETS)
  ...
  foreach(ix RANGE ${targets_len-1})
    ...
    # priority.
    if(DEFINED GAPT_OVERRIDE_CRATE_TYPE_ARGS)
      foreach(override_crate_name override_crate_types IN ZIP_LISTS ${override_crate_name_list_ref}
                                  ${override_crate_types_list_ref})
    ...
    if(TARGET "${target_name}"
       AND ("staticlib" IN_LIST kinds
            OR "cdylib" IN_LIST kinds
            OR "bin" IN_LIST kinds))
      message(
        WARNING "Failed to import Rust crate ${target_name} (kind: `${target_kind}`) because a target "
    ...
    if("staticlib" IN_LIST kinds OR "cdylib" IN_LIST kinds)
      # Explicitly set library names have always been forbidden from using dashes (by cargo). Starting with Rust 1.79,
      # names inherited from the package name will have dashes replaced by underscores too. Corrosion will thus replace
      # dashes with underscores, to make the target name consistent independent of the Rust version. `bin` target names
      # are not affected. See https://github.com/corrosion-rs/corrosion/issues/501 for more details.
      ...
      if(archive_byproducts)
        _corrosion_copy_byproducts(${target_name} ARCHIVE_OUTPUT_DIRECTORY "${cargo_build_out_dir}"
                                   "${archive_byproducts}")
      endif()
      if(shared_lib_byproduct)
        _corrosion_copy_byproducts(${target_name} LIBRARY_OUTPUT_DIRECTORY "${cargo_build_out_dir}"
                                   "${shared_lib_byproduct}")
      endif()
      if(pdb_byproduct)
        _corrosion_copy_byproducts(${target_name} "PDB_OUTPUT_DIRECTORY;LIBRARY_OUTPUT_DIRECTORY"
    ...
    elseif("bin" IN_LIST kinds)
      ...
      _corrosion_initialize_properties(${target_name})
      _corrosion_add_bin_target("${workspace_manifest_path}" "${target_name}" "bin_byproduct" "pdb_byproduct")
      ...
      _add_cargo_build(
        ...
        # Optional
        ${no_linker_override})
      _corrosion_copy_byproducts(${target_name} RUNTIME_OUTPUT_DIRECTORY "${cargo_build_out_dir}" "${bin_byproduct}")
      if(pdb_byproduct)
        _corrosion_copy_byproducts(${target_name} "PDB_OUTPUT_DIRECTORY;RUNTIME_OUTPUT_DIRECTORY"
                                   "${cargo_build_out_dir}" "${pdb_byproduct}")
      endif()
      list(APPEND corrosion_targets ${target_name})
      set_property(TARGET "${target_name}" PROPERTY COR_CARGO_PACKAGE_NAME "${package_name}")
  ...
  if(NOT corrosion_targets)
    ...
    set_target_properties(${corrosion_targets} PROPERTIES INTERFACE_COR_PACKAGE_MANIFEST_PATH
                                                          "${package_manifest_path}")
  endif()
  set(${out_created_targets}
      "${corrosion_targets}"
      PARENT_SCOPE)

endfunction()

# Add all cargo targets defined in the packages defined in the Cargo.toml manifest at `MANIFEST_PATH`.
function(_generator_add_cargo_targets)
  ...
  cmake_parse_arguments(GGC "${options}" "${one_value_args}" "${multi_value_args}" ${ARGN})
  ...
  foreach(ix RANGE ${pkgs_len-1})
    ...
    # probably this loop is not necessary at all, since when using --no-deps, the contents of packages should already be
    ...
    if(NOT DEFINED pkg_is_ws_member)
      # Since we pass `--no-deps` to cargo metadata now,  I think this situation can't happen, but lets check for it
      # anyway, just to discover any potential issues. If nobody complains for a while, it should be safe to remove this
      # check and the previous loop, which should speed up the configuration process.
