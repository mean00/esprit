...
option(Rust_RUSTUP_INSTALL_MISSING_TARGET "Use Rustup to automatically install missing targets instead of giving up"
       OFF)
...
function(_findrust_version_ok ACTUAL_VERSION OUT_IS_OK)
  if(DEFINED Rust_FIND_VERSION_RANGE)
    ...
    if(("${ACTUAL_VERSION}" VERSION_GREATER_EQUAL "${Rust_FIND_VERSION_RANGE_MIN}")
       AND ("${ACTUAL_VERSION}" ${COMPARSION_OPERATOR} "${Rust_FIND_VERSION_RANGE_MAX}"))
      set("${OUT_IS_OK}"
          TRUE
          PARENT_SCOPE)
    else()
...
function(_corrosion_strip_target_triple input_triple_or_path output_triple)
  # If the target_triple is a path to a custom target specification file, then strip everything except the filename from
  # `target_triple`.
...
function(_corrosion_parse_target_triple target_triple out_arch out_vendor out_os out_env)
  ...
  # The vendor part may be left out from the target triple, and since `env` is also optional, we determine if vendor is
  # present by matching against a list of known vendors.
  set(known_vendors
      "apple"
  ...
  # vendor is optional - We detect if vendor is present by matching against a known list of vendors. The next field is
  # the OS, which we assume to always be present, while the last field is again optional and contains the environment.
  string(REGEX MATCH "^([a-z0-9_\.]+)-((${known_vendors_joined})-)?([a-z0-9_]+)(-([a-z0-9_]+))?$" whole_match
               "${target_triple}")
  if((NOT whole_match) AND (NOT CORROSION_NO_WARN_PARSE_TARGET_TRIPLE_FAILED))
    message(
      WARNING "Failed to parse target-triple `${target_triple}`."
...
function(_corrosion_determine_libs_new target_triple out_libs out_flags)
  ...
  execute_process(
    COMMAND ${CMAKE_COMMAND} -E env "CARGO_BUILD_RUSTC=${Rust_COMPILER_CACHED}" ${Rust_CARGO_CACHED} rustc --verbose
            --color never --target=${target_triple} -- --print=native-static-libs
  ...
  if(cargo_build_result)
    ...
    if(cargo_build_error_message MATCHES "native-static-libs: ([^\r\n]+)\r?\n")
      ...
      foreach(lib ${libs_list})
        # merge -framework;lib -> "-framework lib" as CMake does de-duplication of link libraries, and -framework prefix
        # is required
      ...
      # We leave it up to the C/C++ executable that links in the Rust static-library to determine which version of the
      # msvc runtime library it should select.
...
if(NOT "${Rust_TOOLCHAIN}" STREQUAL "$CACHE{Rust_TOOLCHAIN}")
  ...
  set(Rust_TOOLCHAIN
      ${Rust_TOOLCHAIN}
      CACHE STRING "Requested rustup toolchain" FORCE)
endif()

set(_RESOLVE_RUSTUP_TOOLCHAINS_DESC "Indicates whether to descend into the toolchain pointed to by rustup")
set(Rust_RESOLVE_RUSTUP_TOOLCHAINS
    ON
    CACHE BOOL ${_RESOLVE_RUSTUP_TOOLCHAINS_DESC})
...
if(DEFINED Rust_TOOLCHAIN)
  ...
  if(NOT Rust_RUSTUP)
    if(NOT "${Rust_FIND_QUIETLY}")
      message(WARNING "CMake variable `Rust_TOOLCHAIN` specified, but `rustup` was not found. "
                      "Ignoring toolchain and looking for a Rust toolchain not managed by rustup.")
    endif()
  endif()
else()
  # If we aren't definitely using a rustup toolchain, look for rustc first - the user may have a toolchain installed via
  # a method other than rustup higher in the PATH, which should be preferred. However, if the first-found rustc is a
  # rustup proxy, then we'll revert to finding the preferred toolchain via rustup.

  # Uses `Rust_COMPILER` to let user-specified `rustc` win. But we will still "override" the user's setting if it is
  # pointing to `rustup`. Default rustup install path is provided as a backup if a toolchain cannot be found in the
  # user's PATH.
  ...
  if(DEFINED Rust_COMPILER)
    ...
    if(NOT (EXISTS "${_Rust_COMPILER_TEST}" AND NOT IS_DIRECTORY "${_Rust_COMPILER_TEST}"))
      set(_ERROR_MESSAGE "Rust_COMPILER was set to `${Rust_COMPILER}`, but this file does " "not exist.")
    ...
    if(NOT EXISTS "${_Rust_COMPILER_TEST}")
      ...
      set(_ERROR_MESSAGE
          "`rustc` not found in PATH or `${_cargo_bin_dir}`.\n"
  ...
  execute_process(
    COMMAND ${CMAKE_COMMAND} -E env RUSTUP_FORCE_ARG0=rustup "${_Rust_COMPILER_TEST}" --version
    ...
    if(_USER_SPECIFIED_RUSTC)
      message(
        WARNING "User-specified Rust_COMPILER pointed to rustup's rustc proxy. Corrosion's "
                ...
                "user-specified Rust_COMPILER will be discarded in favor of the default " "rustup-managed toolchain.")
...
# At this point, the only thing we should have evaluated is a path to `rustup` _if that's what the best source for a
# Rust toolchain was determined to be_.
if(NOT Rust_RUSTUP)
  set(Rust_RESOLVE_RUSTUP_TOOLCHAINS
      OFF
      CACHE BOOL ${_RESOLVE_RUSTUP_TOOLCHAINS_DESC} FORCE)
endif()
...
foreach(_VAR ${_Rust_USER_VARS})
  if(DEFINED "${_VAR}")
    set(${_VAR}_CACHED
        "${${_VAR}}"
        CACHE INTERNAL "Internal cache of ${_VAR}")
  else()
    unset(${_VAR}_CACHED CACHE)
  endif()
endforeach()

# Discover what toolchains are installed by rustup, if the discovered `rustc` is a proxy from `rustup` and the user
# hasn't explicitly requested to override this behavior, then select either the default toolchain, or the requested
# toolchain Rust_TOOLCHAIN
if(Rust_RESOLVE_RUSTUP_TOOLCHAINS)
  execute_process(COMMAND "${Rust_RUSTUP}" toolchain list --verbose OUTPUT_VARIABLE _TOOLCHAINS_RAW)
  ...
  foreach(_TOOLCHAIN_RAW ${_TOOLCHAINS_RAW})
    if(_TOOLCHAIN_RAW
      ...
      execute_process(COMMAND "${_TOOLCHAIN_PATH}/bin/rustc" --version OUTPUT_VARIABLE _TOOLCHAIN_RAW_VERSION)
      if(_TOOLCHAIN_RAW_VERSION MATCHES "rustc ([0-9]+)\\.([0-9]+)\\.([0-9]+)(-nightly)?")
        ...
        # We need this variable to determine the default toolchain, since `foreach(... IN ZIP_LISTS ...)` requires CMake
        # 3.17. As a workaround we define this variable to lookup the version when iterating through the
        # `_DISCOVERED_TOOLCHAINS` lists.
  ...
  set(Rust_RUSTUP_TOOLCHAINS
      "${_DISCOVERED_TOOLCHAINS}"
      CACHE INTERNAL "List of available Rustup toolchains")
  ...
  set(Rust_RUSTUP_TOOLCHAINS_VERSION
      ...
      CACHE INTERNAL "List of the rust toolchain version corresponding to the toolchain at the same index in \
  ...
  set(Rust_TOOLCHAIN
      "${_TOOLCHAIN_SELECTED}"
      CACHE STRING "The rustup toolchain to use")
...
if(NOT EXISTS "${Rust_COMPILER_CACHED}")
  set(_NOT_FOUND_MESSAGE
      "The rustc executable was not found. " "Rust not installed or ~/.cargo/bin not added to path?\n"
...
if(Rust_RESOLVE_RUSTUP_TOOLCHAINS)
  set(_NOT_FOUND_MESSAGE
      ...
      "if cargo was not built. " "Please manually specify the path to a compatible `cargo` by setting `Rust_CARGO`.")
  find_program(
    Rust_CARGO_CACHED cargo
    HINTS "${_RUST_TOOLCHAIN_PATH}/bin"
    NO_DEFAULT_PATH)
  # note: maybe can use find_package_handle_standard_args here, if we remove the _CACHED postfix. not sure why that is
  # here...
  if(NOT EXISTS "${Rust_CARGO_CACHED}")
    _findrust_failed(${_NOT_FOUND_MESSAGE})
  endif()
  set(Rust_TOOLCHAIN_IS_RUSTUP_MANAGED
      TRUE
      CACHE INTERNAL "" FORCE)
else()
  set(_NOT_FOUND_MESSAGE
      "Failed to find `cargo` in PATH and `${_RUST_TOOLCHAIN_PATH}/bin`.\n"
      "Please ensure cargo is in PATH or manually specify the path to a compatible `cargo` by " "setting `Rust_CARGO`.")
...
# todo: don't set cache variables here, but let find_package_handle_standard_args do the promotion later.
if(_CARGO_VERSION_RAW MATCHES "cargo ([0-9]+)\\.([0-9]+)\\.([0-9]+)")
  set(Rust_CARGO_VERSION_MAJOR
      "${CMAKE_MATCH_1}"
      CACHE INTERNAL "" FORCE)
  set(Rust_CARGO_VERSION_MINOR
      "${CMAKE_MATCH_2}"
      CACHE INTERNAL "" FORCE)
  set(Rust_CARGO_VERSION_PATCH
      "${CMAKE_MATCH_3}"
      CACHE INTERNAL "" FORCE)
  set(Rust_CARGO_VERSION
...
elseif(_CARGO_VERSION_RAW MATCHES "([0-9]+)\\.([0-9]+)\\.([0-9]+)")
  ...
  _findrust_failed("Failed to parse cargo version. `cargo --version` evaluated to (${_CARGO_VERSION_RAW}). "
...
if(_RUSTC_VERSION_RAW MATCHES "rustc ([0-9]+)\\.([0-9]+)\\.([0-9]+)(-nightly)?")
  set(Rust_VERSION_MAJOR
      "${CMAKE_MATCH_1}"
      CACHE INTERNAL "" FORCE)
  set(Rust_VERSION_MINOR
      "${CMAKE_MATCH_2}"
      CACHE INTERNAL "" FORCE)
  set(Rust_VERSION_PATCH
      "${CMAKE_MATCH_3}"
      CACHE INTERNAL "" FORCE)
  set(Rust_VERSION
...
if(_RUSTC_VERSION_RAW MATCHES "host: ([a-zA-Z0-9_\\-]*)\n")
  ...
  set(Rust_CARGO_HOST_TARGET_CACHED
      "${Rust_DEFAULT_HOST_TARGET}"
      CACHE STRING "Host triple")
else()
  _findrust_failed(
...
if(_RUSTC_VERSION_RAW MATCHES "LLVM version: ([0-9]+)\\.([0-9]+)(\\.([0-9]+))?")
  set(Rust_LLVM_VERSION_MAJOR
      "${CMAKE_MATCH_1}"
      CACHE INTERNAL "" FORCE)
  set(Rust_LLVM_VERSION_MINOR
      "${CMAKE_MATCH_2}"
      CACHE INTERNAL "" FORCE)
  # With the Rust toolchain 1.44.1 the reported LLVM version is 9.0, i.e. without a patch version. Since cmake regex
  # does not support non-capturing groups, just ignore Match 3.
  set(Rust_LLVM_VERSION_PATCH
      "${CMAKE_MATCH_4}"
      CACHE INTERNAL "" FORCE)
  set(Rust_LLVM_VERSION
      "${Rust_LLVM_VERSION_MAJOR}.${Rust_LLVM_VERSION_MINOR}.${Rust_LLVM_VERSION_PATCH}"
      CACHE INTERNAL "" FORCE)
elseif(NOT Rust_FIND_QUIETLY)
  message(
    WARNING "Failed to parse rustc LLVM version. `rustc --version --verbose` evaluated to:\n${_RUSTC_VERSION_RAW}")
endif()
...
if(NOT Rust_CARGO_TARGET_CACHED)
  ...
  if(WIN32)
    ...
    if(NOT DEFINED _CARGO_ARCH)
      ...
      # https://learn.microsoft.com/en-us/windows/win32/winprog64/wow64-implementation-details When cross-compiling the
      # user is expected to supply the value, so we match more variants.
    ...
    # The MSVC Generators will always target the msvc ABI. For other generators we check the compiler ID and compiler
    # target (if present) If no compiler is set and we are not cross-compiling then we just choose the default rust host
    # target.
    if(DEFINED MSVC
    ...
    elseif(("${CMAKE_C_COMPILER_ID}" MATCHES "Clang$" OR "${CMAKE_CXX_COMPILER_ID}" MATCHES "Clang$")
           AND ("${CMAKE_CXX_COMPILER_TARGET}" MATCHES "-gnu(llvm)?$" OR "${CMAKE_C_COMPILER_TARGET}" MATCHES
                                                                         "-gnu(llvm)?$"))
      if("${Rust_VERSION}" VERSION_GREATER_EQUAL "1.79")
        set(_CARGO_ABI gnullvm)
      else()
        message(
          WARNING "Your selected C/C++ compilers suggest you want to use the -gnullvm"
    ...
    elseif(NOT "${CMAKE_CROSSCOMPILING}" AND "${Rust_DEFAULT_HOST_TARGET}" MATCHES "-msvc$")
      # We first check if the gnu branches match to ensure this fallback is only used if no compiler is enabled.
      set(_CARGO_ABI msvc)
    else()
      message(WARNING "Could not determine the target ABI. Please specify `Rust_CARGO_TARGET` manually.")
    endif()

    if(DEFINED _CARGO_ARCH
       AND DEFINED _CARGO_VENDOR
       AND DEFINED _CARGO_ABI)
      set(Rust_CARGO_TARGET_CACHED
...
if(Rust_TOOLCHAIN_IS_RUSTUP_MANAGED)
  execute_process(COMMAND rustup target list --toolchain "${Rust_TOOLCHAIN}" OUTPUT_VARIABLE AVAILABLE_TARGETS_RAW)
  ...
  if("${Rust_CARGO_TARGET_CACHED}" IN_LIST AVAILABLE_TARGETS)
    ...
    if(NOT ("${Rust_CARGO_TARGET_CACHED}" IN_LIST INSTALLED_TARGETS))
      if(Rust_RUSTUP_INSTALL_MISSING_TARGET)
        ...
        execute_process(COMMAND "${Rust_RUSTUP}" target add --toolchain ${Rust_TOOLCHAIN} ${Rust_CARGO_TARGET_CACHED}
                        RESULT_VARIABLE target_add_result)
        if(NOT "${target_add_result}" EQUAL "0")
          message(
            FATAL_ERROR
              "Target ${Rust_CARGO_TARGET_CACHED} is not installed for toolchain "
        ...
        message(
          FATAL_ERROR
            "Target ${Rust_CARGO_TARGET_CACHED} is not installed for toolchain ${Rust_TOOLCHAIN}.\n"
...
if(Rust_CARGO_TARGET_CACHED STREQUAL Rust_DEFAULT_HOST_TARGET)
  set(Rust_CROSSCOMPILING
      FALSE
      CACHE INTERNAL "Rust is configured for cross-compiling")
else()
  set(Rust_CROSSCOMPILING
      TRUE
      CACHE INTERNAL "Rust is configured for cross-compiling")
...
_corrosion_parse_target_triple("${Rust_CARGO_HOST_TARGET_CACHED}" rust_host_arch rust_host_vendor rust_host_os
                               rust_host_env)

set(Rust_CARGO_TARGET_ARCH
    "${rust_arch}"
    CACHE INTERNAL "Target architecture")
set(Rust_CARGO_TARGET_VENDOR
    "${rust_vendor}"
    CACHE INTERNAL "Target vendor")
set(Rust_CARGO_TARGET_OS
    "${rust_os}"
    CACHE INTERNAL "Target Operating System")
set(Rust_CARGO_TARGET_ENV
    "${rust_env}"
    CACHE INTERNAL "Target environment")

set(Rust_CARGO_HOST_ARCH
    "${rust_host_arch}"
    CACHE INTERNAL "Host architecture")
set(Rust_CARGO_HOST_VENDOR
    "${rust_host_vendor}"
    CACHE INTERNAL "Host vendor")
set(Rust_CARGO_HOST_OS
    "${rust_host_os}"
    CACHE INTERNAL "Host Operating System")
set(Rust_CARGO_HOST_ENV
    "${rust_host_env}"
    CACHE INTERNAL "Host environment")
...
if(NOT DEFINED CACHE{Rust_CARGO_TARGET_LINK_NATIVE_LIBS})
  ...
  # In very recent corrosion versions it is possible to override the rust compiler version per target, so to be totally
  # correct we would need to determine the libraries for every installed Rust version, that the user could choose from.
  # In practice there aren't likely going to be any major differences, so we just do it once for the target and once for
  # the host target (if cross-compiling).
  set(Rust_CARGO_TARGET_LINK_NATIVE_LIBS
      "${required_native_libs}"
      CACHE INTERNAL "Required native libraries when linking Rust static libraries")
  set(Rust_CARGO_TARGET_LINK_OPTIONS
      "${required_link_flags}"
      CACHE INTERNAL "Required link flags when linking Rust static libraries")
endif()
...
if(Rust_CROSSCOMPILING AND NOT DEFINED CACHE{Rust_CARGO_HOST_TARGET_LINK_NATIVE_LIBS})
  ...
  set(Rust_CARGO_HOST_TARGET_LINK_NATIVE_LIBS
      "${host_libs}"
      CACHE INTERNAL "Required native libraries when linking Rust static libraries for the host target")
  set(Rust_CARGO_HOST_TARGET_LINK_OPTIONS
      "${host_flags}"
      CACHE INTERNAL "Required linker flags when linking Rust static libraries for the host target")
endif()

# Set the input variables as non-cache variables so that the variables are available after `find_package`, even if the
# values were evaluated to defaults.
foreach(_VAR ${_Rust_USER_VARS})
...
if(NOT TARGET Rust::Rustc)
  ...
  set_property(TARGET Rust::Rustc PROPERTY IMPORTED_LOCATION "${Rust_COMPILER_CACHED}")

  add_executable(Rust::Cargo IMPORTED GLOBAL)
  set_property(TARGET Rust::Cargo PROPERTY IMPORTED_LOCATION "${Rust_CARGO_CACHED}")
  set(Rust_FOUND true)
endif()
