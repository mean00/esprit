...
impl SerialTxOnly {
    ...
    pub fn new(instance: i32, dma: bool, buffered: bool) -> Self {
    ...
    /// Set the baud rate.
    pub fn set_speed(&mut self, speed: i32) -> bool {
    ...
    pub fn transmit(&mut self, data: &[u8]) -> bool {
        unsafe { rn_serial_c::lnserial_tx_transmit(self.raw, data.len() as i32, data.as_ptr()) }
    }

    /// Low‑level raw write (bypasses any buffering).  Returns `true` on success.
    pub fn raw_write(&mut self, data: &[u8]) -> bool {
        unsafe { rn_serial_c::lnserial_tx_raw_write(self.raw, data.len() as i32, data.as_ptr()) }
    }
}
...
/// # Callback safety
///
/// The callback fires from **interrupt context**.  The trampoline recovers
...
impl SerialRxTx {
    ...
    pub fn new(instance: i32, rx_buffer_size: i32, dma: bool) -> Self {
    ...
    /// Blocking transmit.  Returns `true` on success.
    pub fn transmit(&mut self, data: &[u8]) -> bool {
        unsafe { rn_serial_c::lnserial_rx_transmit(self.raw, data.len() as i32, data.as_ptr()) }
    }

    /// Non‑blocking transmit.  Returns the number of bytes accepted (may be
    /// less than `data.len()`).
    pub fn transmit_no_block(&mut self, data: &[u8]) -> i32 {
        unsafe {
            rn_serial_c::lnserial_rx_transmit_no_block(self.raw, data.len() as i32, data.as_ptr())
        }
    }
    ...
    pub fn read(&mut self, buf: &mut [u8]) -> i32 {
        unsafe { rn_serial_c::lnserial_rx_read(self.raw, buf.len() as i32, buf.as_mut_ptr()) }
    }

    /// Attach (or replace) an event handler.
    ///
    /// The previous handler (if any) is **leaked** — the C driver owns the
    ...
    ///
    /// Returns the number of contiguous bytes available, and sets `*to` to
