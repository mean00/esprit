...
//!
//! ```ignore
//! use rust_esprit::MultiPulse;
//!
//! let mut mp = MultiPulse::new(Pin::PB1, 100_000).unwrap();
...
impl MultiPulse {
    ...
    pub fn new(pin: Pin, tick_fq_hz: i32) -> Option<Self> {
        let inner = unsafe { mp::ln_multi_pulse_create(pin, tick_fq_hz) };
        if inner.is_null() {
            None
        } else {
    ...
    pub fn fire(&self, pulse1_ms: i32, gap_ms: i32, pulse2_ms: i32) {
        unsafe { mp::ln_multi_pulse_fire(self.inner, pulse1_ms, gap_ms, pulse2_ms) }
    }

    /// Block until the pulse sequence completes.
    pub fn wait_done(&self) {
        unsafe { mp::ln_multi_pulse_wait_done(self.inner) }
    }

    /// Abort a running pulse sequence.
    pub fn stop(&self) {
        unsafe { mp::ln_multi_pulse_stop(self.inner) }
    }
}

impl Drop for MultiPulse {
    fn drop(&mut self) {
        unsafe { mp::ln_multi_pulse_delete(self.inner) }
    }
}